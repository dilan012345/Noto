# Noto: Notes app
<p>
Minimal notes app in a material 3 style
</p>

https://dilan012345.github.io/Noto/

Minimum SDK: 33
(Android 13+ / Tiramisu)

## Features
> [!NOTE]
> UNFINISHED

## Installation

1. Download the latest APK from Releases
2. Install correct version
3. Use phone file Explorer to install the app

## Screenshots
<img width="300" alt="1000039225" src="https://github.com/user-attachments/assets/213ffeb7-6865-4384-b660-39f23dae2740" />
<img width="300" alt="1000039224" src="https://github.com/user-attachments/assets/e26d87da-af24-4b8a-b4c6-697fe577f511" />
<img width="300" alt="1000039223" src="https://github.com/user-attachments/assets/9eac685c-fcc4-4f98-8698-94b96c73f8c3" />

## Changelog
### v2.5.6-beta
Room implementation 

### v1.5.6-beta
Import notes page established
3 badge row at scrollable content bottom

### v1.4.6-beta
Transition between pages

### v1.4.5-beta
Conversion to navController + navHost

### v1.4.4-beta
Purple UI switched to green


[![GitHub](https://img.shields.io/badge/GitHub-Repository-black?logo=github)](https://github.com/dilan012345/Noto/tree/main)
[![Download](https://img.shields.io/badge/Download-APK-green?logo=android)](//)
[![Latest release](https://img.shields.io/github/v/release/dilan012345/Noto?style=flat-square&label=release&color=00afd1)](https://github.com/dilan012345/Noto/releases)



mostly finished, began work on Aero; will publish release when I can be bothered
